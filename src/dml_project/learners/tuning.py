...
def make_main_learner(name: str, random_state: int | None = None):
    ...
    Args:
        name: Learner identifier. Supported values are ``"ols"``,
    ...
    if name == "ols":
        return OLSLearner()
    if name == "lasso":
        return LassoCVLearner(cv=config.N_FOLDS, random_state=random_state)
    if name == "elastic_net":
        return ElasticNetCVLearner(cv=config.N_FOLDS, random_state=random_state)
